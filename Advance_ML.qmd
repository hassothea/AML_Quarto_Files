...
## Example: Email spam filter ✉️ {.incremental}
...
### Example: Email spam filter ✉️ {visibility="uncounted"}
::: {.panel-tabset}
### Target
```{python}
#| echo: true
...
- Suitable graphic[$^{\text{📚}}$]{style="font-size: 0.7em"}: [histogram]{style="color:green"}, density, boxplot, violinplot...
...
<div class="hline"></div>
<div class="footnote">$^{\text{📚}}$ [Chapter 2 & 3, *Handbook of Data Visualization*, Chen et al. (2008).](https://haralick.org/DV/Handbook_of_Data_Visualization.pdf)</div>
...
::: {.incremental}
- $\mathbb{P}(E),\mathbb{P}(H)$: How likely for $E,H$ to occur.
- $\mathbb{P}(H|E)$: How likey for $H$ to occur given that $E$ occured.
:::
...
$$\mathbb{P}(Y=1|X=\text{x})=\frac{\mathbb{P}^{\small\text{📚}}(X=\text{x}|Y=1)\times\mathbb{P}(Y=1)}{\mathbb{P}(X=\text{x})}.$$

- $\mathbb{P}(Y=1|X=\text{x})^{\small\text{📚}}$ allows us to classify email $x$:
...
<div class="hline"></div>
<div class="footnote">
$^{\text{📚}}$ [Here, $\mathbb{P}$ may refer to PDF (continous) or PMF (discrete) according to the type of $\text{x}$.](https://www.sas.upenn.edu/~fdiebold/NoHesitations/BookAdvanced.pdf)<br>
$^{\text{📚}}$ [Chapter 2, *The Elements of Statistical Learning*, Hastie et al. (2008).](https://www.sas.upenn.edu/~fdiebold/NoHesitations/BookAdvanced.pdf)</div>
...
<div class="footnote">$^{\text{📚}}$ [Chapter 4, *Introduction to Statistical Learning with R*, James et al. (2021).](https://www.sas.upenn.edu/~fdiebold/NoHesitations/BookAdvanced.pdf)</div>
...
## Imbalanced data ⚠️ {.incremental auto-animate=true auto-animate-easing="ease-in-out" auto-animate-delay="0"}
:::{.columns}
:::{.style="font-size:0.8em"}
...
## Imbalanced data ⚠️ {auto-animate=true auto-animate-easing="ease-in-out" auto-animate-delay="0" visibility="uncounted"}
:::{.columns}
...
## Imbalanced data ⚠️ {auto-animate=true auto-animate-easing="ease-in-out" auto-animate-delay="0"}
...
## Imbalanced data ⚠️ (Summary) {auto-animate=true auto-animate-easing="ease-in-out" auto-animate-delay="0"}
...
## Imbalanced data ⚠️ (to explore) {auto-animate=true auto-animate-easing="ease-in-out" auto-animate-delay="0"}
- Sampling methods:
    - Oversampling: random, SMOTE, SMOTE SVM, ADASYN...
    - Undersampling: random, new miss, CNN, Tomek Links...
- Weight adjustment methods (nonparametric methods)
...
# 2. Linear & Quadratic Discriminant Analysis (LDA & QDA) {background-color="#15797A"}
...
fig.update_layout(title = "1D Gaussian R.V",
                  width = 550,
                  height = 450,
...
fig.update_layout(title = "2D Gaussian Vectors",
                  width = 600,
                  height = 500,
                  legend=dict(
                  yanchor="top",
...
:::{.column width="60%"}
- Simulate $x_1,\dots,x_{100}$ with
$x_i\sim{\cal N}\left(\begin{pmatrix}
           0 \\
           0
...
$f_X(\text{x})=\frac{e^{-\frac{1}{2}(\text{x}-\mu)^t\Sigma^{-1}(\text{x}-\mu)}}{\sqrt{(2\pi)^d|\Sigma|}}.$
    ...
    - $\mathbb{V}(X)=\mathbb{E}[(X-\mu)(X-\mu)^t]=\Sigma$ (positive definite).
:::
:::
...
fig0.update_layout(title = "Gaussian models",
                  scene_camera=camera,
                  width = 420,
                  height = 510)
fig0.show()
...
::: {.callout-tip}
### Boundary Decision of QDA
- Boundary decision of class $k$ and $j$ in QDA contains inputs $\text{x}$ such that $\delta_k(\text{x})=\delta_j(\text{x})$. 
...
<div class="hline"></div>
<div class="footnote">$^{\text{📚}}$ [Read Chapter 4 of *The Elements of Statistical Learning*, Hastie et al. (2008).](https://www.sas.upenn.edu/~fdiebold/NoHesitations/BookAdvanced.pdf)</div>
...
- In LDA, discriminant function is simplied to $$\delta_k(\text{x})=\log(\pi_k)-\frac{1}{2}(\text{x}-\mu_k)^t\Sigma^{-1}(\text{x}-\mu_k).$$

:::{.columns}
::: {.callout-tip}
### Boundary Decision of LDA
- In this case, the boundary decision takes `Linear Form` of $\text{x}: \text{x}^tv+c=0$, where $v\in\mathbb{R}^d$ and $c\in\mathbb{R}$ and depend on $\pi_k,\pi_j,\mu_k,\mu_j$ and $\Sigma$.
:::
:::
...
<div class="hline"></div>
<div class="footnote">$^{\text{📚}}$ [Chapter 4, *The Elements of Statistical Learning*, Hastie et al. (2008).](https://www.sas.upenn.edu/~fdiebold/NoHesitations/BookAdvanced.pdf)</div>
